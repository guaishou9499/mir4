...
参数：无
返回：int
函数： actor_obj: battle_status()

说明：寻路类型(1(NPC寻路) 2(任务寻路))
参数：无
返回：int
函数： actor_obj: auto_move_type()

说明：自动类型(3(自动采集)  2(自动战斗)
参数：无
返回：int
函数： actor_obj: auto_type()

说明： 瞬移状态（应该是红名）
参数：无
返回：int
函数： actor_obj: can_teleport()
...
参数：无
返回：int
函数： actor_obj: red_name_status()

说明：采集物品点数
参数：无
返回：int
函数： actor_obj: gather_item_life()

说明：采集物品最大点数
参数：无
返回：int
函数： actor_obj: gather_item_maxlife()

说明：是否可采集
参数：无
返回：int
函数： actor_obj: can_gather()

说明：采集玩家ID
参数：无
返回：int
函数： actor_obj: gather_player_id()

说明：雷状态
参数：无
返回：int
函数： actor_obj: mine_status()

说明：检测采集ID是否在无效列表
参数：无
返回：int
函数： actor_obj: is_gather_id_invalid()

说明：检查采集对象CD时间
参数：无
返回：int
函数： actor_obj: has_gather_item_cd()

说明：是否有效(坐标判断)
参数：无
返回：int
函数： actor_obj: can_moveto()

说明：采集物品类型
参数：无
返回：int
函数： actor_obj: gather_type()

说明：读取buff 剩余时间
参数：无
返回：int
函数： actor_obj: buff_time()

说明：是否无敌
参数：无
返回：int
函数： actor_obj: is_super_man()

说明：是否有指定状态
参数：buff_id
返回：bool
函数： actor_obj: has_buff(int buff_id)
...
参数2：y坐标
返回：float
函数： actor_obj: dist_xy(float tx, float ty)
...
返回：int
函数：auction_unit:new()
local auction_obj = actor_unit:new()

说明：初始化
参数：对象
返回：int
函数：auction_obj:init(dwObject) 

说明：销毁对象
参数：无
返回：int
函数：auction_obj:delete()
...
参数1：按钮对象
返回：bool
函数：ui_unit.btn_click(ULONG_PTR object) 

说明：控件点击(%80控件可用)
参数1：按钮对象
返回：bool
函数：ui_unit.do_click(ULONG_PTR object) 

说明：公共挂件确认
参数1：无
返回：bool
函数：ui_unit.common_widget_confirm() 

说明：公共挂件取消
参数1：无
返回：bool
函数：ui_unit.common_widget_cancel() 

说明：获取UI管理对象
参数1：无
返回：int
函数：ui_unit.get_ui_action_manager() 

说明：根据ID取UI对象
参数1：id
返回：int
函数：ui_unit.get_ui_action_byid(int id) 

说明：根据名称取UI对象
参数1：sName
返回：int
函数：ui_unit.get_ui_action_byname(string sName) 

说明：获取顶层管理对象
参数1：无
返回：int
函数：ui_unit.get_widget_manager() 

说明：读取顶层控件
参数1：sName
参数2：bVisible 
返回：int
函数：ui_unit.get_parent_widget(string sName, bool bVisible = true) 

说明：读取子控件
参数1：父控件
参数2：名称
返回：int
函数：ui_unit.get_child_widget(ULONG_PTR parent, std::string sName) 

说明：Txt控件文本
参数1：控件对象
返回：string
函数：ui_unit.get_txt_control_text(ULONG_PTR object) 

说明：公共弹窗内容
参数1：无
返回：string
函数：ui_unit.get_popup_info_text() 

说明：初使化
参数1：dwObject
返回：bool
函数：ui_unit.init(int dwObject) 

```
## /Mir4G/游戏单元(game_unit)
```text
## 静态函数
...
参数：无
返回：bool
函数：great_obj: can_update()

说明：建筑是否在升级中
参数：无
返回：bool
函数：great_obj: is_updating()

说明：是否完成升级
参数：无
返回：bool
函数：great_obj: is_finish_update()

说明：升阶目标数
参数：无
返回：int
函数：great_obj: tar_num()

说明：序号取目标id
参数：nIdx
返回：int
函数：great_obj: tar_id(int nIdx)

说明：序号取目标名称
参数：nIdx
返回：string
函数：great_obj: tar_name(int nIdx)

说明：目标是否已完成
参数：nIdx
返回：bool
函数：great_obj: tar_is_finish(int nIdx)

说明：取升阶建筑数量
参数：nIdx
返回：int
函数：great_obj: tar_build_num(int nIdx)

说明：序号取升阶建筑ID
参数：nIdx
返回：int
函数：great_obj: tar_build_id(int nIdx)

说明：目标建筑主ID
参数：nIdx
返回：int
函数：great_obj: tar_build_main_id(int nIdx)

说明：需要建筑等级
参数：nIdx
返回：int
函数：great_obj: tar_build_need_level(int nIdx)

说明：升阶建筑名称
参数：nIdx
返回：string
函数：great_obj: tar_build_name(int nIdx)

说明： 建筑是否达到要求
参数：nIdx
返回：boll
函数：great_obj: build_meet_requirements(int nIdx)
...
```
## /Mir4G/帮派技能单元(guild_skill_unit)
```text
## 静态函数

说明：请求技能信息(大页从上到下 1 到 4)
参数1：nIdx
返回：bool
函数：guild_skill_unit.req_skill_info(int nIdx) 

说明：升级帮会技能
参数1：nId
返回：bool
函数：guild_skill_unit.up_skill(int nId) 

说明：查询技能数据
参数1：nId
返回：bool
函数：guild_skill_unit.query_skill_info(int nId) 

说明：取技能DataSet
参数1：无
返回：int
函数：guild_skill_unit.get_guild_devinfo_data_set() 

说明：技能ID取技能数据
参数1：nId
返回：int
函数：guild_skill_unit.get_guild_devinfo_data(int nId) 

说明：取技能升级信息指针
参数1：nId
参数2：nLevel
返回：int
函数：guild_skill_unit.get_guild_devinfo_uplevel_ptr(int nId, int nLevel) 

说明：取技能UI信息（主要取技能名称资源ID）
参数1：nId
返回：int
函数：guild_skill_unit.get_guild_devinfo_ui_ptr(int nId) 

说明：取帮会技能列表
参数1：无
返回：int
函数：guild_skill_unit.skill_list() 
...
参数：无
返回：int
函数：guild_skill_unit:new()
local guild_skill_obj = guild_skill_unit:new()

说明：初始化
参数：对象
返回：int
函数guild_skill_obj :init(dwObject) 

说明：销毁对象
参数：无
返回：int
函数：guild_skill_obj :delete()

说明：id
参数：无
返回：int
函数：guild_skill_obj :id()

说明：名称
参数：无
返回：string
函数：guild_skill_obj :name()

说明：等级
参数：无
返回：int
函数：guild_skill_obj :level()

说明：取当前经验
参数：无
返回：int
函数：guild_skill_obj :cur_exp()

说明：升级需要经验
参数：无
返回：int
函数：guild_skill_obj :up_level_exp()

说明：是否可以升级
参数：无
返回：bool
函数：guild_skill_obj :can_up_level()

说明：升级剩余时间
参数：无
返回：int
函数：guild_skill_obj :up_level_remain_time()

说明：升级所需时间
参数：无
返回：int
函数：guild_skill_obj :up_level_time()
...
function example:test_guild_skill_unit()
	...
	-- 查询技能信息
	guild_skill_unit.query_skill_info(0x18A91)
	sleep(2000)
	local list = guild_skill_unit.skill_list()
	local guild_skill_obj = guild_skill_unit:new()

	local obj = guild_skill_unit.get_guild_devinfo_data(0x18A91)
	if guild_skill_obj:init(obj) then 
		xxmsg(string.format('%16X   %08X    %04u    %u    %u    %-6s   %u----%u      %s',
			obj,
			guild_skill_obj:id(),
...
返回：int
函数：guild_unit:new()
local guild_obj = guild_unit_unit:new()

说明：初始化
参数：对象
返回：int
函数 guild_obj :init(dwObject) 
...
参数：无
返回：int
函数：guild_obj :member_num()

说明：最大人数
参数：无
返回：int
函数：guild_obj :max_member_num()

说明：是否满员
参数：无
返回：bool
函数：guild_obj :is_full()

说明：战力需求
参数：无
返回：bool
函数：guild_obj :need_combat_power()
...
参数：无
返回：bool
函数：item_obj :equip_is_use()

说明：装备类型
参数：无
返回：int
函数：item_obj :equip_type()

说明：强化等级
参数：无
返回：int
函数：item_obj :equip_enhanced_level()

说明：装备战力
参数：无
返回：int
函数：item_obj :equip_combat_power()

说明：职业
参数：无
返回：int
函数：item_obj :equip_job()
...
参数：无
返回：int
函数：make_res_unit:new()
local make_res_obj = make_res_unit:new()

说明：初始化
参数：对象
返回：int
函数 make_res_obj :init(dwObject) 

说明：销毁对象
参数：无
返回：int
函数：make_res_obj :delete()
...
参数：无
返回：int
函数：make_res_obj :make_id()

说明：制作地方 类型 7D0   随机制作的装备 7D1 随身制作材料
参数：无
返回：int
函数：make_res_obj :make_pos_type()

说明：取名称U8
参数：无
返回：string
函数：make_res_obj :name()
...
参数：无
返回：int
函数：pet_obj :is_unlock()

说明：是否可召唤
参数：无
返回：bool
函数：pet_obj :can_summon()

说明：取宝物列表
参数：无
返回：int
函数：pet_obj :pet_item_list()
...
参数：无
返回：int
函数：quest_obj :main_type()

说明：每日任务次数
参数：无
返回：int
函数：quest_obj :daily_num()
...
参数：无
返回：bool
函数：quest_obj :can_accept()

说明：战斗力
参数：无
返回：int
函数：quest_obj :combat_power()
...
参数：无
返回：string
函数：quest_obj :map_name()

说明：当前目标数
参数：无
返回：int
函数：quest_obj :tar_num()

说明：最大目标数
参数：无
返回：int
函数：quest_obj :tar_max_num()
...
参数：无
返回：int
函数：quest_obj :tar_type()

说明：是否完结
参数：无
返回：bool
函数：quest_obj :is_over()

说明：是否完成
参数：无
返回：bool
函数：quest_obj :is_finish()

说明：是否自动
参数：无
返回：bool
函数：quest_obj :is_auto()

说明：委托任务采集对象资源ID
参数：无
返回：int
函数：quest_obj :entrust_gather_resid()
...
参数：无
返回：int
函数：relation_unit:new()
local relation_obj = relation_unit:new()

说明：初始化
参数：对象
返回：int
函数 relation_obj :init(dwObject) 

说明：销毁对象
参数：无
返回：int
函数：relation_obj :delete()
...
参数：无
返回：string
函数：relation_obj :main_name()

说明：名称U8
参数：无
返回：string
函数：relation_obj :name()

说明：miaoshu
参数：无
返回：string
函数：relation_obj :desc()

说明：任务地图
参数：无
返回：int
函数：relation_obj :map_id()

说明：地图名称
参数：无
返回：string
函数：relation_obj :map_name()
...
参数：无
返回：bool
函数：relation_obj :is_kill_monster()
...
参数：无
返回：int
函数：skill_obj :study_level()

说明：行为数量（有行为就可以发送技能）
参数：无
返回：int
函数：skill_obj :action_num()

说明：序号取行为
参数：nIdx
返回：int
函数：skill_obj :get_action_byidx(int nIdx)
...
```
## /Mir4G/队伍单元(team_unit)
```text
## 静态函数
说明：邀请组队
参数1：nPlayerId
返回：bool
函数：team_unit.invite(int nPlayerId) 

说明：接受组队
参数1：nTeamId
参数2：nInvitePlayerId
返回：bool
函数：team_unit.accept(int nTeamId, int nInvitePlayerId) 
...
参数1：无
返回：bool
函数：team_unit.disband() 

说明：创建组队 注：模式（0 无1普通2精英3迷宫4秘谷5魔方阵6秘境峰） 成员模式 0: 5人 ,1 :15人
参数1：nMode
参数1：nMemberMode
参数1：nCombatPower
返回：bool
函数：team_unit.create(int nMode, int nMemberMode, int nCombatPower) 

说明：刷新队伍列表（0x65 首领  0x66普通讨伐）
参数1：nType
返回：bool
函数：team_unit.update_team_list(int nType) 
...
参数1：讨伐ID
返回：bool
函数：team_unit.join_team(int nTeamId, int nType, int TaoFaId) 

说明：队长开始讨伐
参数1：讨伐Id
返回：bool
函数：team_unit.start_taofa(int TaoFaId) 

说明：讨伐完成确认
参数1：nTaoFaTyep
返回：bool
函数：team_unit.finish_taofa(int nTaoFaTyep) 

说明：创建讨伐
参数1：讨伐类型
参数1：讨伐ID
返回：bool
函数：team_unit.create_taofa(int nTaoFaTyep, int nTaoFaId) 

说明：是否可进入讨伐
参数1：讨伐类型
返回：bool
函数：team_unit.can_taofa_ticket(int nTaoFaTyep) 

说明：取队伍ID
参数1：无
返回：int
函数：team_unit.team_id() 

说明：是否有队伍
参数1：无
返回：bool
函数：team_unit.has_team() 

说明：取队伍人数
参数1：无
返回：int
函数：team_unit.member_num() 

说明：自己是否为队长
参数1：无
返回：bool
函数：team_unit.hero_is_leader() 

说明：取队伍列表
参数1：ParytType
参数2：TaoFaId
返回：bool
函数：team_unit.party_list(int ParytType, int TaoFaId) 
...
参数：无
返回：bool
函数：team_obj :is_leader()

说明：讨伐队伍ID
参数：无
返回：int
函数：team_obj :party_id()

说明：讨伐类型（0x66 普通讨伐）（0x65 首领讨伐）
参数：无
返回：int
函数：team_obj :party_type()

说明：讨伐ID
参数：无
返回：int
函数：team_obj :taofa_id()

说明：队伍所需要战力
参数：无
返回：int
函数：team_obj :have_combat_power()
