...
function item_ent.set_quick_slot()
    ...
    for i = 1, #set_hp_list do
        ...
        if item_num > 0 then
            local set_pos = false
            local hp_status = actor_unit.get_game_option_status(0xB)
            if hp_status < 100 then
                actor_unit.set_game_option(0xB, 100)
                decider.sleep(500)
            end
            if local_player:hp() * 100 / local_player:max_hp() < hp_status then
...
-- 通过物品名物品数量获取物品信息
function item_ent.get_item_info_by_name(item_name)
    local item_info = {}
    local item_list = item_unit.list(-1)
    for _, obj in pairs(item_list) do
        if item_ctx:init(obj) then
...
function item_ent.use_unsealing_box()
    ...
    if idx1 > 10 or idx1 == 10 and idx2 >= 12 then
        -- 取解封槽数量
        for i = 1, unsealing_unit.get_unsealing_solt_num() do
            -- 判断是否解封完成
...
function item_ent.sell_item()
    ...
    for i = 1, #all_bag_item_info do
        if all_bag_item_info[i].is_bind then
            ...
            if not sell then
                if string.find(item_name, '魔石') and not string.find(item_name, '箱') then
                    local stone_list = {
                        ['英雄活力魔石'] = { level = 10 },
                        ['英雄专注魔石'] = { level = 9 },
                        ['英雄摧毁魔石'] = { level = 8 },
                    ...
                    if not stone_list[item_name] then
                        sell = true
                    end
                end
            end
...
-- 通过物品名获取非绑定物品信息
function item_ent.get_not_bind_item_info_by_name(item_name)
    local item_info = {}
    local item_list = item_unit.list(-1)
    local item_num = 0
    for _, obj in pairs(item_list) do
        if item_ctx:init(obj) then
            local num = item_ctx:num()
            if num > 0 and item_name == item_ctx:name() and not item_ctx:is_bind() then
                item_num = item_num + num
                item_info = {
                    -- 系统id
                    sys_id = item_ctx:sys_id(),
