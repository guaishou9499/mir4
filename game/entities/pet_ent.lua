...
function pet_ent.auto_pet(war_pet_type)
    local idx1,idx2 = quest_res.get_main_task_idx()
    if idx1 > 6 or idx1 == 6 and idx2 >= 6 then
        pet_ent.get_can_use_pet()
        pet_ent.go_war_pet(war_pet_type)
...
-- 自动佩戴宠物
function pet_ent.go_war_pet(war_pet_type)
    local pet = {}
    local pet_list = pet_res.PET_LIST
    if war_pet_type == '采集' then
        pet_list = pet_res.GATHER_PET_LIST
    end
    ...
    local idx = 1
    if local_player:level() >= 15  then
        idx = 2
    end
    for pos = 0, idx do
        if not table.is_empty(pet[pos + 1]) then
            -- 判断是否上架
            local shangjia = false
            for pos2 = 0, idx do
                if pet_unit.get_warpet_obj_byidx(pos2) == pet_unit.get_pet_object_byid(pet[pos + 1].id) then
                    shangjia = true
                end
            end
            if not shangjia then
                local idx_id = pet_unit.get_warpet_obj_byidx(pos)
                if idx_id ~= pet_unit.get_pet_object_byid(pet[pos + 1].id) then
                    if idx_id ~= 0 then
                        pet_unit.cancel_war_pet(idx_id, pos)
                        decider.sleep(2000)
                    end
