...
function gather_ent.get_near_best_gather(actor_info_list, gather_type)
    if table.is_empty(actor_info_list) then
        return {}
    end
    if #actor_info_list == 1 then
        return actor_info_list[1]
    end
    local map_name = actor_unit.map_name()
    local gather_info = actor_res.GATHER_RES[map_name][gather_type]
    for i = 3, 1, -1 do
        for j = 1, #actor_info_list do
            local gather_level = gather_info[actor_info_list[j].res_id].gather_type or 0
            if gather_level == i then
                return actor_info_list[j]
            end
        end
    end
    return actor_info_list[1]
...
-- 采集模块
function gather_ent.gather_mod(map_name,level,gather_type)
    level = level or 3
    gather_type = gather_type or '采集'
    ...
    local break_func = function()
        local gather_info = gather_ent.get_gather_info_by_res_id2(res_id, map_name,level,gather_type)
        if not table.is_empty(gather_info) then
            return true
        end
    ...
    if map_name == actor_unit.map_name() then
        if local_player:status() ~= 14 then
            ...
            while decider.is_working() do
                ...
                local gather_info = gather_ent.get_gather_info_by_res_id2(res_id, map_name,level,gather_type)
                if not table.is_empty(gather_info) then
                    if func.is_rang_by_point(local_player:cx(), local_player:cy(), gather_info.cx, gather_info.cy, 500) then
                        actor_unit.gather_ex(gather_info.sys_id)
                        decider.sleep(1000)
                    else
...
-- 通过采集物res_id获取采集物信息
function gather_ent.get_gather_info_by_res_id2(res_id, map_name,level,gather_type)
     gather_type = gather_type or '采集'
    local actor_info_list = {}
    local actor_list = actor_unit.list(4)
    for i = 1, #actor_list do
        ...
        if actor_ctx:init(obj) then
            ...
            if can_gather then
                ...
                if actor_res.GATHER_RES[map_name][gather_type][actor_ctx:res_id()].gather_type < level then
