...
function equip_ent.auto_equip()
    ...
    if idx1 > 4 or idx1 == 4 and idx2 >= 12 then
...
equip_ent.auto_build_equip = function()
    ...
    for _, v in pairs(equip_list) do
        ...
        if not this.build_equip_2(build_equip) then
...
function equip_ent.build_equip_1(make_equip_name)
    ...
    if not table.is_empty(build_info) then
        ...
        if need_move then
            local str = string.format('[打造蓝装] 去比奇青龙打造%s',make_equip_name)
            map_ent.auto_move(101001010, -3735, -12346, 618, str, 200)
        end
        if not_sys_id then
            if need_move and local_player:dist_xy(-3735, -12346) < 300 then
...
equip_ent.enhanced_equip = function(equip_obj, enhanced_level)
    ...
    local idx1,idx2 = quest_res.get_main_task_idx()
    if idx1 > 11 or idx1 == 11 and idx2 >= 21 then
        while decider.is_working() do
            if item_ctx:init(equip_obj) then
                ...
                decider.sleep(1000)
                for i = 1, 10 do
                    local str = string.format('强化[%s]到[%s/%s]...%s', name, equip_enhanced_level,enhanced_level, i)
                    trace.output(str)
