...
function shop_ent.super_preload()
    ...
    this.wi_secret_map_auto_buy_item = decider.run_interval_wrapper('秘境峰自动购买物品', this.secret_map_auto_buy_item, 1000 * 60 * 10)

end

function shop_ent.auto_buy_item()
    local idx1,idx2 = quest_res.get_main_task_idx()
    if idx1 == 9  and idx2 == 21 then
        return false
    elseif idx1 == 9  and idx2 == 22 then
        return false
    end
    local buy_list = {
...
function shop_ent.secret_map_auto_buy_item()
    local buy_list = {
        ['小型生命值恢复药水'] = {min_num  = 20,max_num = 200,buy_min_num = 100},
        ['中型生命值恢复药水'] = {min_num  = 200,max_num = 1000,buy_min_num = 200},
        ['大型生命值恢复药水'] = {min_num  = 200,max_num = 1000,buy_min_num = 200},
    ...
    local hp_name, map_name = game_ent.use_hp_type()
    if hp_name == '中型生命值恢复药水' then
        shop_ent.secret_auto_buy_item(hp_name, buy_list[hp_name].min_num, buy_list[hp_name].max_num, buy_list[hp_name].buy_min_num)
        shop_ent.secret_auto_buy_item(map_name, buy_list[map_name].min_num, buy_list[map_name].max_num, buy_list[map_name].buy_min_num)
    end
end
    ...
    while decider.is_working() do
        ...
        if local_player:hp() > 0 then
            ...
            if actor_unit.map_name() == npc_map_name then
                if local_player:dist_xy(npc_x, npc_y) < 300 then
                    -- 获取npc信息
                    ...
                    map_ent.auto_move(npc_map_name, npc_x, npc_y, npc_z, str, 300)
                end
            else
                map_ent.auto_move(npc_map_name, npc_x, npc_y, npc_z, str, 300)
            end
...
function shop_ent.mf_auto_buy_item(item_name, min_num, max_num, buy_min_num)
    ...
    while decider.is_working() do
        ...
        if not dungeon_ent.in_mf_map() then
            break
        end
        if local_player:hp() > 0 then
            local str = '去买' .. item_name .. ' - 移动中'
...
function shop_ent.secret_auto_buy_item(item_name, min_num, max_num, buy_min_num)
    local map_name = actor_unit.map_name()
    local merchant_type = '药水商人'
    if item_name == '花酒' then
        merchant_type = '酒商'
    ...
    local npc_info = shop_res.MERCHANT['秘境峰1层'][merchant_type]
    if table.is_empty(npc_info) then
        return false, '该地图附近没有商人npc'
    end
    local near_npc_coordinate = this.get_near_shop_npc(npc_info.npc_pos)
    local npc_map_name = npc_info.map_name
    local npc_x, npc_y, npc_z = near_npc_coordinate.x, near_npc_coordinate.y, near_npc_coordinate.z
    local price = npc_info.items[item_name].price
    local item_id = npc_info.items[item_name].item_id
    local shop_id = npc_info.items[item_name].shop_id
    local npc_name = npc_info.npc_name
...
--- 获得最近药水商人
function shop_ent.get_near_shop_npc(npc_pos_tab)
    if not table.is_empty(npc_pos_tab) then
        table.sort(npc_pos_tab, function(a, b)
            
            return local_player:dist_xy(a.x, a.y) < local_player:dist_xy(b.x, b.y)
        end)
    end

    return npc_pos_tab[1]
end
