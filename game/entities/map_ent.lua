...
function map_ent.super_preload()
    this.wi_fast_move = decider.run_interval_wrapper('自动瞬移', this.fast_move, 1000 * 10)
...
function map_ent.auto_move_to(cx, cy, cz, map_id)
    cx , cy = map_ent.get_circle_pos(cx, cy, 360, 100)
...
-- 根据角度，距离，坐标 获取坐标
------------------------------------------------------------------------------------
map_ent.get_circle_pos = function(x,y,n_angle,dis)
    if not n_angle then
        return x,y
    end
...
-- 瞬移
function map_ent.fast_move(cx, cy, cz)
    local random = math.random(-50, 50)
    cx = cx + random
    cy = cy + random
    cz = cz + random
    if map_ent.is_move() then
        map_ent.auto_move_to(cx, cy, cz)
        decider.sleep(500)
    end
    xxmsg('瞬移')
    actor_unit.fast_move(cx, cy, cz)
    decider.sleep(1000)
end
...
function map_ent.auto_move(map_id, x, y, z, str, dist, not_ride, break_func)
    ...
    while decider.is_working() do
        ...
        if map_id == 300140 and actor_unit.map_id() == map_id then
            if local_player:dist_xy(70, 6929) < 500 then
                map_ent.wi_fast_move(x,y,z)
            end
        end
        if not_ride then
            -- TODO:不骑马
        ...
        if map_ent.move_lag(60) then
            ...
            if not escape_pos then
                ...
                map_ent.fast_move(escape_pos.x, escape_pos.y, escape_pos.z)
            end
            decider.sleep(5000)
        end
        decider.sleep(1000)
...
function map_ent.lag_in_map()
    if not game_unit.is_loding() then
        map_ent.lag_in_map_time = 0
        return
    end
    if not map_ent.lag_in_map_time then
        map_ent.lag_in_map_time = os.time()
    end
    if os.time() - map_ent.lag_in_map_time > 150 then
        trace.output('过图超时-重启')
        main_ctx:end_game()
    end
end

-- 移动到指定位置
function map_ent.auto_move2(map_id, x, y, z, str, dist, not_ride, break_func)
    local b_ret = false
    if type(map_id) == 'string' then
        map_id = map_res.GET_MAP_ID[map_id]
    end
