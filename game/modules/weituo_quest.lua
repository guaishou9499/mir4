...
local weituo_quest = {
    VERSION = '202015.1',
    AUTHOR_NOTE = "-[weituo_quest module - 202015.1]-",
    MODULE_NAME = '委托模块',
    -- 设置脚本版本
...
---@type weituo_quest_ent
local weituo_quest_ent = import('game/entities/weituo_quest_ent')
...
-- 轮循函数列表
weituo_quest.poll_functions = {}

------------------------------------------------------------------------------------
-- 预载函数(重载脚本时)
weituo_quest.super_preload = function()

end

-------------------------------------------------------------------------------------
-- 预载处理
weituo_quest.preload = function()
...
weituo_quest.entry = function()
    ...
    do
        ...
        -- 执行轮循任务
        weituo_quest.looping()

        quest_ent.do_weituo_task()
...
-------------------------------------------------------------------------------------
-- 模块超时处理
weituo_quest.on_timeout = function()
...
-------------------------------------------------------------------------------------
-- 卸载处理
weituo_quest.unload = function()
    --xxmsg('weituo_quest.unload')
end

-------------------------------------------------------------------------------------
-- 效验登陆异常
weituo_quest.check_login_error = function()
...
function weituo_quest.__tostring()
    return this.MODULE_NAME
end

weituo_quest.__index = weituo_quest

function weituo_quest:new(args)
    local new = { }
    ...
    -- 设置元表
    return setmetatable(new, weituo_quest)
end

-------------------------------------------------------------------------------------
-- 返回对象
return weituo_quest:new()
