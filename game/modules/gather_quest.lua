...
local gather_quest = {
    VERSION = '202015.1',
    AUTHOR_NOTE = "-[gather_quest module - 202015.1]-",
    MODULE_NAME = '采集任务',
    -- 设置脚本版本
...
this.eval_ifs = {
    ...
    -- [其它] 特殊情况才用(可选)
    is_working = function()
        return switch_ent.gather_quest() and not switch_ent.mf_fb() and not switch_ent.weituo_task() and not switch_ent.secret_fb()
...
-- 轮循函数列表
gather_quest.poll_functions = {}

------------------------------------------------------------------------------------
-- 预载函数(重载脚本时)
gather_quest.super_preload = function()

end

-------------------------------------------------------------------------------------
-- 预载处理
gather_quest.preload = function()
...
gather_quest.entry = function()
    ...
    do
        -- 执行轮循任务
        gather_quest.looping()

        gather_ent.gather_quest()
        decider.sleep(100)
       return false
    end
    return ret_b
...
-------------------------------------------------------------------------------------
-- 模块超时处理
gather_quest.on_timeout = function()
...
-------------------------------------------------------------------------------------
-- 卸载处理
gather_quest.unload = function()
    --xxmsg('gather_quest.unload')
end

-------------------------------------------------------------------------------------
-- 效验登陆异常
gather_quest.check_login_error = function()
...
function gather_quest.__tostring()
    return this.MODULE_NAME
end

gather_quest.__index = gather_quest

function gather_quest:new(args)
    local new = { }
    ...
    -- 设置元表
    return setmetatable(new, gather_quest)
end

-------------------------------------------------------------------------------------
-- 返回对象
return gather_quest:new()
